...
function ProjectCardLinks({ project }: { project: ProjectMeta }) {
  return (
...
function buildCardClassName(isWip: boolean): string {
  const base = "flex flex-col h-full transition-colors duration-200";
  if (!isWip) return `${base} hover:border-primary/50`;

  return `${base} border-emerald-500/30 hover:border-emerald-400/60 shadow-[0_0_15px_-3px] shadow-emerald-500/10`;
}
...
function ProjectCard({ project }: { project: ProjectMeta }) {
  ...
  return (
    <Card className={buildCardClassName(isWip)}>
